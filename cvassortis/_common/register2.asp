...
<div id="holder">
	...
	<input type="hidden" name="exp_edu_continue" value="1">
		...
		<h3><span class="left">&nbsp;</span><span class="right">&nbsp;</span><% =GetLabel(sCvLanguage, "Education") %></h3>
	...
	%>
		...
		If iExpEduDiplomaID=arrEduTypeID(i) Then
			Response.Write("<option value=" & arrEduTypeID(i) & " selected>" & EducationTypeTitleByID(i) & "</option>")
		Else 
			Response.Write("<option value=" & arrEduTypeID(i) & ">" & EducationTypeTitleByID(i) & "</option>")
		End If
		Next %>
		</select>&nbsp;&nbsp;<span class="fcmp">*</span></td>
		</tr>
		...
		<a href="javascript:AddEducation(0);"><img class="button first" src="<% =sHomePath %>image/bte_<% If iExpEduID>0 Then %>save<% Else %>add<% End If %>qualif.gif" name="Add this education" alt="Add this education to the list of your qualifications" border=0></a>
