...
<% ShowMessageEnd %><br/>
		...
		<h3><% =GetLabel(sCvLanguage, "Native languages") %></h3>
...
<% ShowMessageEnd %><br/>
		...
		<div class="box search blue">
		<h3><% =GetLabel(sCvLanguage, "Foreign languages") %></h3>
		<table class="search_form" width="100%" cellspacing=0 cellpadding=0 border=0>
	<% ' List of languages
	Set objTempRs=GetDataRecordsetSPWithConn(objConnCustom, "usp_ExpCvvLanguageSelect", Array( _
		Array(, adInteger, , iExpertID), _
...
End If %>
		...
		<a href="javascript:AddLanguage(0);" class="red-button w125 under-right-col" title="Append this record about language knowledge"><% if iExpLngID>0 then %>save<% Else %>add<% End If %> language</a>
