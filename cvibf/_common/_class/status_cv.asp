...
Class CStatusCVList
	...
	Public Function LoadData
		LoadDataProc "usp_StatusCVSelect", Array()
	End Function
	...
	Public Function LoadDataProc(AProcedure, AParams)
		...
		If Not objTempRs.Eof Then
			...
			While Not objTempRs.Eof
				...
				Items(FCount).ID=objTempRs("id_Status")
				Items(FCount).Name=objTempRs("stsNameEng")
	...
	Public Sub ShowSelectItems(ASelectedItem)
		...
		If FCount>0 Then
			For i=0 To FCount-1
			%>
			<option value="<% =Items(i).ID %>"<% If ASelectedItem=Items(i).ID Then %> selected<% End If %>><% =Items(i).Name %></option>
			<% 
			Next
		End If
	End Sub	
