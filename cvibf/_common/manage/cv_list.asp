...
-->	
	...
	<% If iShowRemoved=0 Then
		...
		Response.Write "</p></td>" 
		%>
		<td><small><a href="../register/comments.asp?uid=<% =objTempRs("uid_Expert") %>"><img src="<% =sHomePath %>image/vn_updt.gif" width="15" height="15" align="left" hspace="6" vspace="0" border="0" alt="Edit comments for <% =objTempRs("psnLastName") & ", " & objTempRs("psnFirstName") & " " & objTempRs("psnMiddleName") & " (" & objTempRs("ptlName") & ")" %>"></a><% =objTempRs("expComments") %></small></td>
		<%
...
%>
<tr class="grid"><td colspan="8"><p class="mt">Total: <b><%=ShowEntityPlural(iTotalExpertsNumber, "expert", "experts", "&nbsp;") %></b><% If Len(sSearchString)>0 Then Response.Write " matching search criteria"%></p></td></tr>
</table>
</div><br />
<% ShowNavigationPages iCurrentPage, iTotalPages, sParams %>
