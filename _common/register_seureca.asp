...
If Request.Form()>"" Then
	...
	' Save custom fields
		...
		If sApplicationName="expert" Then
			objResult2=UpdateRecordSP("usp_ExpertProfileLanguageUpdate", Array(_
				Array(, adInteger, , iExpertID), _
				Array(, adVarChar, 3, sCvLanguage)))
		Else
			objResult2=UpdateRecordSP("usp_ExpertProfileCustomUpdate", Array(_
				Array(, adInteger, , iExpertID), _
				Array(, adVarChar, 3, sCvLanguage), _
				Array(, adVarWChar, 150, sCvFolder)))
		End If
	On Error GoTo 0
...
<% ShowMessageEnd %>
	...
	<input type="hidden" name="id_Person" value="<% =iPersonID %>">
		<% InputBlockSpace 4 %>
		<% InputBlockElementLeftStart %><p class="ftxt">CV language</p><% InputBlockElementLeftEnd %><% InputBlockElementMiddle %>
		<% InputBlockElementRightStart %>&nbsp;&nbsp;<select name="cv_language" size="1" style="width:130px;">
		<% 
		Response.Write "<option value=""Eng""" 
		If sCvLanguage="Eng" Then Response.Write " selected"
		Response.Write ">English</option>"
		Response.Write "<option value=""Fra""" 
		If sCvLanguage="Fra" Then Response.Write " selected"
		Response.Write ">French</option>"
		%>
		</select><% InputBlockElementRightEnd %>
	<% If sApplicationName<>"expert" Then %>
		<% InputBlockElementLeftStart %><p class="ftxt">CV type</p><% InputBlockElementLeftEnd %><% InputBlockElementMiddle %>
		<% InputBlockElementRightStart %>&nbsp;&nbsp;<select name="cv_type" size="1" style="width:130px;">
		<option value=""> </option>
		<% 
		Response.Write "<option value=""Seureca""" 
		If sCvFolder="Seureca" Then Response.Write " selected"
		Response.Write ">Seureca</option>"
		Response.Write "<option value=""Veolia Group""" 
		If sCvFolder="Veolia Group" Then Response.Write " selected"
		Response.Write ">Veolia Group</option>"
		Response.Write "<option value=""Other Company""" 
		If sCvFolder="Other Company" Then Response.Write " selected"
		Response.Write ">Other Company</option>"
		Response.Write "<option value=""Independent""" 
		If sCvFolder="Independent" Then Response.Write " selected"
		Response.Write ">Independent</option>"
		%>
		</select><% InputBlockElementRightEnd %>
	<% End If %>
		<% InputBlockSpace 4 %>
  ...
  <!-- Personal information -->
	...
	<% InputBlockHeader "100%" %>
		...
		<% InputBlockElementRightStart %>&nbsp;&nbsp;<select name="exp_title" size="1" style="width:110px;">
