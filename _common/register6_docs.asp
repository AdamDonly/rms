...
<% ShowTopMenu %>
<% If sApplicationName<>"backoffice" Or iUserAdmin=1 Then %>
...
<tr><td width="85%" valign="top">
	<%
	' Clients with "send all communication to a client directly"
	If (iUserAdmin = 1 And (sHomePath="/lfs/")) Then
		If bEmailExpertAccountSent = 2 Then
			ShowMessageStart "info", 550
				%>
				<p>Login details haven't been sent yet!</p>
				<p><a href="register_confirm.asp?id=<% =iExpertID %>">Send personal login and password to the client now</a>.</p>
				<%
			ShowMessageEnd
		ElseIf bEmailExpertAccountSent = 1 And sAction<>"resent" Then
			ShowMessageStart "info", 600
				%>
				<p>Login details have been sent already.</p>
				<p>If required, you can</p>
				<p><a href="register_confirm.asp?id=<% =iExpertID %>">Resend the welcome message and the login details</a> in case the expert/client didn't receive it or</p> 
				<p><a href="password_confirm.asp?id=<% =iExpertID %>">Resend just the login details</a> to the expert/client.</p>
				<%
			ShowMessageEnd
		ElseIf bEmailExpertAccountSent = 0 Then
			ShowMessageStart "info", 600
				%>
				<p><a href="register_confirm.asp?id=<% =iExpertID %>">Send the welcome message and the login details</a></p> 
				<p><a href="password_confirm.asp?id=<% =iExpertID %>">Send just the login details</a>.</p>
				<%
		End If
	ElseIf sApplicationName="external" Or (iUserAdmin = 1 And (sHomePath="/tripleline/")) Then
		If bEmailExpertAccountSent = 2 Then
...
<td width="20%" valign="top">
	...
	<tr><td width="100%" bgcolor="#EAEAEA" background="<%=sHomePath%>image/fbox_ttlbg.gif"><p class="fbox" align="center">Supporting documents</p></td></tr>
	...
	<div align="center"><a href="../register/document.asp?id=<%=iExpertID%>&document=0"><img src="<% =sHomePath %>image/bte_upload.gif" vspace="4" border="0" alt="Upload document"></a></div>
	<% ShowFeatureBoxFooterWithFormFooter %>
	<br>
	
	<% If iUserAdmin=1 Or sApplicationName="external" Then %>
	<% ShowFeatureBoxHeader("CV options") %>
	<% If bCvMultiLanguageActive = cMultiLanguageEnabled Then %>
		<% 
		Dim objCvRs, bCvMultiLanguageShow
		Set objCvRs = GetCvIdDetails(iExpertID) 
		If Not objCvRs.Eof Then
			If objCvRs("id_Expert2")>0 Then
				bCvMultiLanguageShow = 1
				%>
				<p class="sml" style="padding: 2px 5px;"><b>CV in other language(s):</b></p>
				<%
			End If

			While Not objCvRs.Eof
			%>
				<p class="sml" style="padding: 2px 5px;"><a href="<% =sScriptFileName & ReplaceUrlParams(sParams, "id=" & objCvRs("id_Expert2")) %>"><b><% =ReplaceIfEmpty(dictLanguage.Item(Trim(objCvRs("Lng2"))), objCvRs("Lng2")) %></b></a></p>
				<%
				objCvRs.MoveNext
			WEnd

			If bCvMultiLanguageShow Then
				ShowFeatureBoxDelimiter
			End If
		End If
		Set objCvRs = Nothing
