...
<script language="JavaScript" src="/_scripts/js/lib_gip.js"></script>
<% InsertScrollStyles %>
...
<% ShowMessageStart "info", 490 %>
<% =GetLabel(sCvLanguage, "Specify professional experiences (GIP)") %>
<% ShowMessageEnd %>
...
<!--#include file="../_common/expWorkFields.asp"-->
	...
	<td width=578 bgcolor="<%=colFormHeaderTop%>" valign="top">
		...
		<tr><td width=170><p class="ftxt"><% =GetLabel(sCvLanguage, "Funding agency") %><br /><span class="sml2">(<% =GetLabel(sCvLanguage, "only for international projects") %>)</span></p></td>
		<td bgcolor="<%=colFormBodyRight%>" width=1><img src="x.gif" width=1 height=24></td>
	...
	<td width=1 bgcolor="<% =colFormHeaderSplitter %>"></td></tr>
		...
		<div class="mini_wraper">
			...
			<%
			' Countries
			ShowCouScrollBox GetLabel(sCvLanguage, "Select project's countries (GIP)"),  "", 1, 0, 1, 1, 0 %>

			<% ShowMessageStart "info", 400 %>
			<% =GetLabel(sCvLanguage, "First click on the sector title (GIP)") %>
			<% ShowMessageEnd %>

			<%
			' Sectors
			ShowSctScrollBox GetLabel(sCvLanguage, "Select project's sub-sectors"),  "", 1, 0, 0 %><br />
		</div>
