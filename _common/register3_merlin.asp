...
Dim bExpPrjNewRecord, iExpPrjID, sExpPrjTitle, sExpPrjGroup, iExpType, sExpPrjStartDate, sExpPrjEndDate, bExpPrjOngoing, sExpPrjOrganisation, sExpPrjPosition, sExpPrjBeneficiary, sExpPrjReferences, sExpPrjDescription, sExpPrjDonor, sPrjDescription
...
If Request.Form()>"" Then
	...
	sExpPrjGroup=Left(CheckString(Request.Form("proj_group")), 255)
	...
	bExpPrjNewRecord=Len(Trim(sExpPrjTitle)) + Len(Trim(sExpPrjGroup)) + Len(Trim(sExpPrjOrganisation)) + Len(Trim(sExpPrjPosition)) + Len(Trim(sExpPrjBeneficiary)) + Len(Trim(sExpPrjReferences)) + Len(Trim(sExpPrjDescription)) + Len(Trim(sExpPrjDonor)) + Len(ReplaceIfEmpty(Trim(sExpPrjStartDate), "")) + Len(ReplaceIfEmpty(Trim(sExpPrjEndDate), ""))
	...
	' Custom updates
	objTempRs=UpdateRecordSP("usp_ExpCvvExperienceCustomUpdate", Array( _
		Array(, adVarChar, 3, "Eng"), _
		Array(, adInteger, , iExpertID), _
		Array(, adInteger, , iExpPrjID), _
		Array(, adVarWChar, 255, sExpPrjGroup)))
...
<% If sApplicationName="expert" Then %>
    if(mmb_cou>50) { alert("<% =GetLabel(sCvLanguage, "You cannot select more than 50 countries for one project") %>"); return; }
<% End If %>
...
<br />
  <!-- Key qualifications -->
	...
	If Not objTempRs.Eof Then
		...
		sExpPrjGroup=objTempRs("wkeInfoGroup")
...
<!--#include file="../_common/expWorkFields.asp"-->
	...
	<td width=578 bgcolor="<%=colFormHeaderTop%>" valign="top">
		...
		<tr><td width=170><p class="ftxt"><% =GetLabel(sCvLanguage, "Experience group") %></td>
		<td bgcolor="<%=colFormBodyRight%>" width=1><img src="x.gif" width=1 height=36></td>
		<td bgcolor="<%=colFormBodyText%>" width=407>
		&nbsp;&nbsp;<input type="text" name="proj_group" size=31 style="width:355px;" maxlength=255 value="<%=sExpPrjGroup%>"></td></tr>
		...
		<td bgcolor="<%=colFormBodyRight%>" width=1><img src="x.gif" width=1 height=24></td>
		<td bgcolor="<%=colFormBodyText%>" width=407>&nbsp;&nbsp;<input type="text" id="exp_ref_name" name="exp_ref_name" size=31 style="width:355px;" maxlength=255 value="<%=sExpRefName%>"></td></tr>

		<tr><td width=170><p class="ftxt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<% =GetLabel(sCvLanguage, "Position") %></td>
		<td bgcolor="<%=colFormBodyRight%>" width=1><img src="x.gif" width=1 height=24></td>
		<td bgcolor="<%=colFormBodyText%>" width=407>&nbsp;&nbsp;<input type="text" id="exp_ref_position" name="exp_ref_position" size=31 style="width:355px;" maxlength=255 value="<%=sExpRefPosition%>"></td></tr>
