...
-->	
	...
	<% While Not objTempRs.Eof And iCurrentRow<objTempRs.PageSize %>
		...
		<td><p class="mt"><a href="<% If sHomePath="/tripleline/" Then %>//cvip.assortis.com/tripleline/backoffice/view/cv_saveec.asp?ftype=doc&id=<% Else %>../register/register6.asp?id=<%End If %><% =objTempRs("id_Expert") %>" target=_blank><% =sExpertFullName %></a></b></p></td>
...
%>
<tr bgcolor="#FFFFFF"><td colspan="11"><p class="mt">Total: <b><%=ShowEntityPlural(iTotalExpertsNumber, "expert", "experts", "&nbsp;") %></b><% If Len(sSearchString)>0 Then Response.Write " matching search criteria"%></p></td></tr>
</table>
</td></tr>
</table>
