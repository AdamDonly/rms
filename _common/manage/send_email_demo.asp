...
<tr bgcolor="#FFFFFF"><td colspan="8"><p class="mt"><b>Send emails to <%=ShowEntityPlural(iTotalExpertsNumber, "expert", "experts", "&nbsp;") %></b><% If sAction="" And InStr(sScriptFullName, "/cvassortis/") Then Response.Write " matching the selected criteria"%><br><br>
	...
	<form action="not for demo" method="post" onSubmit="ConfirmEmail(); return false;">
	<tr height="29">
	<td width="15%"><p class="mt">From</p></td>
	<td width="85%">
	<select size="1" width="85%" name="message_from" id="message_from">
	<option value="">- email@provided.by.client -</option>
	<%
...
-->
	...
	<tr height="29">
	<td>&nbsp;</td>
	<td><input disabled type="submit" value=" &nbsp; &nbsp; Send &nbsp; &nbsp; "> &nbsp; &nbsp;
	<% If iSearchQueryID>0 Then %>
	<input disabled type="button" value=" &nbsp; Cancel &nbsp; " onClick="javascript:window.close();"></td>
	<% Else %>
	<input disabled type="button" value=" &nbsp; Cancel &nbsp; " onClick="javascript:window.location.href='cv_list.asp?<%= Request.QueryString %>'"></td>
	<% End If %>
	</tr>
	</form>
	</table>
