...
Sub ShowIcaExpertsBlock(bSplitedOnPages, iCurrentPage)
	...
	While Not objTempRs.Eof And iCurrentRow < objTempRs.PageSize 
		...
		Dim bMyCompanyCircleExpert
		bMyCompanyCircleExpert = GetExpertCompanyCircleByUid(objTempRs("uid_Expert"), iUserCompanyID, iUserID)

		' Verify if expert is already registered in top experts
		Dim iMyCompanyTopExpertStatus
		iMyCompanyTopExpertStatus = GetExpertCompanyTopExpertByUid(objTempRs("uid_Expert"), iUserCompanyID, iUserID)



		Dim dictTopExpertOfOtherMember
		' check if is a top expert of another member:
		If Not bMyCompanyCircleExpert Then
		...
		If bMyCompanyCircleExpert Then
			If iMyCompanyTopExpertStatus = 1 Then
		...
		ElseIf Not IsNull(dictTopExpertOfOtherMember) Then
			...
			Next
			If Len(tmpMembersForTopExpert) > 0 Then
				sBlockTopExtra = sBlockTopExtra & "<div class=""topExpert_othMember""><img src=""/image/file_top.gif"" width=18 height=17/> " & tmpMembersForTopExpert & " Top Expert</div>"
			End If
		End If
...
Sub ShowExpertMethodologyFlag(AExpertUID, AExpertDB, AExpertID, ACvValidForMemberOrExpert)
	...
	On Error Resume Next
		...
		Set objTempRs3 = GetDataRecordsetSP("usp_Ica_ExpertMethodologySelect", Array( _
		...
		If Not objTempRs3.Eof Then
			If Len(objTempRs3("expMethoSelectedFlag")) > 1 Then
				...
				<td class="value metho_<% =sFlagMetho %>_bg"><p>
				<% If bUserModifyMethodology = True Then %>
				<a href="javascript:editMethodology('<% =AExpertUID %>', '<% =AExpertDB.ID %>', '<% =AExpertID %>');"><img src="/image/modify.gif" hspace="12" align="right"></a>
				<% End If %>
				<img src="/image/metho_<% =sFlagMetho %>.gif" alt="<% =sFlagMetho %>" vspace="2" hspace="3" width="11" height="12" border="0" align="left">
				...
				If Len(sMethoFields) > 0 Then sMethoFields = "<p class=""metho"">" & sMethoFields & sMethoLanguages & "</p>"
				Response.Write sMethoFields
				%>

				<% If bUserAccessMethodologyIbfComments = True Then %>
					<% If Len(objTempRs3("expMethoComments")) > 2 Then %>
						<p class="metho">Comments: <% =HtmlEncode(objTempRs3("expMethoComments")) %></p>
					<% End If %>
				<% End If %>
				<% If bUserAccessMethodologyAsrComments = True Then %>
					<% If Len(objTempRs3("expMethoAsrComments")) > 2 Then %>
						<p class="metho">Comments: <% =HtmlEncode(objTempRs3("expMethoAsrComments")) %></p>
					<% End If %>
				<% End If %>

				<% If objTempRs3("expMethoShowAll") = "1" Then %>
					<% If Len(objTempRs3("expMethoExpComments")) > 2 Then %>
						<p class="metho"><% If Len(objTempRs3("expMethoComments")) > 2 Then %>&nbsp; &nbsp; Expert<% Else %>Expert's comments<% End If %>:
						<% = HtmlEncode(objTempRs3("expMethoExpComments")) %></p>
					<% End If %>

					<% If Len(objTempRs3("expMethoKeywords")) > 2 Then %>
						<p class="metho">Writing experience: <% =HtmlEncode(objTempRs3("expMethoKeywords")) %></p>
					<% End If %>
					<% If Len(objTempRs3("expMethoExpKeywords")) > 2 Then %>
						<p class="metho"><% If Len(objTempRs3("expMethoKeywords")) > 2 Then %>&nbsp; &nbsp; Expert<% Else %>Expert's writing experience<% End If %>:
						<% =HtmlEncode(objTempRs3("expMethoExpKeywords")) %></p>
					<% End If %>


					<% If Len(objTempRs3("expMethoDonors")) > 2 Then %>
						<p class="metho">Donors: <% = HtmlEncode(objTempRs3("expMethoDonors")) %></p>
					<% End If %>
				<% End If %>
