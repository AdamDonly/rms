...
<div class="col666666 itlc botMrgn10">
	...
	If iUserAccessExpertsOtherDbNoAccess = 1 OR iUserAccessExpertsOtherDbRestricted = 1 Then
		...
		%><b>Search among <% =ShowEntityPlural(memberExpertsCount, "expert", "experts", " ") %><% If memberExpertsCount = 0 Then %>s<% End If %> in <% =objUserCompanyDB.DatabaseTitle %> Expert Database<%
...
<!-- Keywords search -->
	<form action="exp_results.asp" method="post" name="RegForm" id="RegForm" onSubmit="Continue(); return false;">
		...
		<h3 style="color: #FFF;margin-top: 0px;font-size: 119%;text-align: left;height: 31px;line-height: 31px;padding: 0 15px;">Keywords search</h3>
  ...
  <!-- Search options -->
		<div class="box search blue">
		<h3 style="color: #FFF;margin-top: 0px;font-size: 119%;text-align: left;height: 31px;line-height: 31px;padding: 0 15px;">Search options</h3>
		<table class="search_form" width="100%" cellspacing=0 cellpadding=0 border=0>
