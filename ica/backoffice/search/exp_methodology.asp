...
<% 
If Not bUserModifyMethodology Then
	Response.Redirect "/"
	Response.End
End If
...
Dim	iExpertMethoSent, _
	...
	sExpertMethoIbfComments, _
	sExpertMethoAsrComments, _
...
If Request.Form() > "" Then
	...
	sExpertMethoIbfComments = CheckStringLength(HtmlEncode(Request.Form("comments")), 10000)
	sExpertMethoAsrComments = CheckStringLength(HtmlEncode(Request.Form("asrcomments")), 10000)
	...
	If bUserAccessMethodologyIbfComments = True Then
		UpdateRecordSP "usp_Ica_ExpertMethodologyFlagFullUpdate2", Array( _
			...
			Array(, adVarWChar, 10000, sExpertMethoIbfComments), _
	...
	If bUserAccessMethodologyAsrComments = True Then
		UpdateRecordSP "usp_Ica_ExpertMethodologyFlagFullUpdate3", Array( _
			Array(, adInteger, , objExpertDB.ID), _
			Array(, adInteger, , iCvID), _
			Array(, adInteger, , sExpertMethoSelectedFlag), _
			Array(, adInteger, , bExpertMethoShowAll), _
			...
			Array(, adInteger, , bExpertMethoPT), _
			Array(, adInteger, , bExpertMethoDE), _
			Array(, adVarWChar, 10000, sExpertMethoAsrComments), _
			Array(, adVarWChar, 10000, sExpertMethoKeywords), _
			Array(, adVarWChar, 10000, sExpertMethoDonors) _
			)
	End If
...
Else
	...
	If Not objTempRs.Eof Then
		...
		sExpertMethoIbfComments = objTempRs("expMethoComments")
		sExpertMethoAsrComments = objTempRs("expMethoAsrComments")
...
<% If bUpdated = False Then %>
	...
	<tr>
		...
		<td colspan="2">
		<div class="value">
			<textarea class="inputarea" name="comments" id="comments" cols="31" style="width: 90%; height: 72px" rows="3" wrap="yes"><% =sExpertMethoIbfComments %></textarea>
		</div>
		</td>
	</tr>
	<% End If %>
	...
	<% If bUserAccessMethodologyAsrComments = True Then %>
	<tr>
		<td valign="top"><p><label class="inline" for="asrcomments">Assortis comments</label><br><small>only &lt;strong&gt; and &lt;br&gt; html tags are allowed</small></p></td>
		<td colspan="2">
		<div class="value">
			<textarea class="inputarea" name="asrcomments" id="asrcomments" cols="31" style="width: 90%; height: 72px" rows="3" wrap="yes"><% =sExpertMethoAsrComments %></textarea>
		</div>
		</td>
	</tr>
	<% End If %>
