...
'
'--------------------------------------------------------------------
Const aSearchKeywordsNoise = "a an and are as at but by for is in it its on of or the through to up"
...
Function GetSearchKeywords()
	Dim sResult
	sResult = Trim(Request.QueryString("txt") + " " + Request.QueryString("srch_queryadd"))
	sResult = Replace(sResult, " AND ", " ")
	sResult = Replace(sResult, " OR ", " ")
	sResult = Replace(sResult, " NOT ", " ")
	sResult = Replace(sResult, " NEAR ", " ")
	...
	If Len(sResult) > 2 Then
		arrSearchKeywordsHighlight = Split(sResult, " ")
	End If

	GetSearchKeywords = sResult
End Function

Function HighlightKeywords(sText, sSearchKeywordsHighlight)
Dim sResult, iKeywordNumber, iNoiseLoop, bNoise
Dim arrSearchKeywordsNoise
 	arrSearchKeywordsNoise = Split(aSearchKeywordsNoise, " ")
	sResult = sText
	iKeywordNumber = 1
	...
	If Len(sSearchKeywordsHighlight)> 2 Then 
		...
		For Each sKeyword in arrSearchKeywordsHighlight
			bNoise = 0
			For iNoiseLoop = 0 To UBound(arrSearchKeywordsNoise)
				If sKeyword = arrSearchKeywordsNoise(iNoiseLoop) Then
					bNoise = 1
				End If
			Next

			If bNoise = 0 Then
				If Len(sKeyword) > 1 And Len(sResult) > 1 Then
					sKeyword = Replace(sKeyword, "*", "\w*")

					objRegEx.Pattern = "\b" & sKeyword & "\b"
					Set colMatch = objRegEx.Execute(sResult)
				
					For Each objMatch In colMatch
						'sResult = objRegEx.Replace(sResult, "<span class=""marktext" & iKeywordNumber & """>" & objMatch.Value & "</span>")
						sResult = Replace(sResult, objMatch.Value, "<span class=""marktext" & iKeywordNumber & """>" & objMatch.Value & "</span>")
					Next 

					If iKeywordNumber < 6 Then 
						iKeywordNumber = iKeywordNumber + 1
					Else 
						iKeywordNumber = 1
					End If
				End If
...
Function HighlightPhrase(sText, sSearchPhraseHighlight)
Dim sResult, iKeywordNumber
	sResult = sText
	iKeywordNumber = 1
	...
	If Len(sSearchPhraseHighlight) > 1 And Len(sResult) > 1 Then 
		objRegEx.Pattern = "\b" & sSearchPhraseHighlight & "\b"
		Set colMatch = objRegEx.Execute(sResult)
...
HighlightPhrase = sResult
End Function

Function HighlightKeywordsOld(sText, sSearchKeywordsHighlight)
Dim sResult, i
sResult=sText
i=1
	If Len(sSearchKeywordsHighlight)>2 Then 
...
Function CheckUserLogin(sScriptNameIn)
	If (Not iUserID>0) _
		...
		Or (sApplicationName="backoffice" 	And sUserType<>sApplicationName And sUserType<>"Admin" And sUserType<>"CV Ibf") _
	...
	Then
		...
		Dim sTempParams
		sTempParams=sScriptFullNameAsParams
		sTempParams=ReplaceUrlParams(sTempParams, "sid=" & sSessionID)
		sTempParams=ReplaceUrlParams(sTempParams, "srv=" & sScriptServerName)
		
		'sUrl=sScriptServerProtocol & sMainServerName & "/asproxy/?sid=" & sSessionID & "&srv=" & sScriptServerName
		sUrl=sScriptServerProtocol & sMainServerName & "/asproxy/" & sTempParams
...
Function CheckExpertID()
	...
	If sApplicationName <> "expert" Then
		...
		On Error Resume Next
			...
			If iMemberAccessExperts = cMemberAccessExpertsOwnOnly Then
				...
				Else
					Set objExpertDB = objUserCompanyDB
				End If
			End If
	...
	If Not IsObject(objExpertDB) Then
		Response.Clear
		Response.Redirect "/"
		Response.End
	End If

	If InStr(sScriptFileName, "cv_copy.asp") <= 0 _
		And InStr(sScriptFileName, "quality.asp") <= 0 _
		And IsIcaUserCompanyCvValid(objExpertDB.Database, iExpertID, objUserCompanyDB.Database) <> 1 _
		And IsIcaUserCompanyCvValid(objExpertDB.Database, iExpertID, objUserCompanyDB.Database) <> 5 _
	Then
		If bAssortisSubscriptionEdbActive And objExpertDB.Database = "assortis" Then 
	...
	If objExpertDB.Database <> objUserCompanyDB.Database And objExpertDB.Database = "assortis" Then 
		If sScriptFileName = "register.asp" _
			Or sScriptFileName = "register2.asp" _
			Or sScriptFileName = "register21.asp" _
			Or sScriptFileName = "register3.asp" _
			Or sScriptFileName = "register4.asp" _
			Or sScriptFileName = "register5.asp" _
		Then
			Response.Redirect "/"
		End If
	End If
...
Function CheckExpertOriginalID()
	...
	objTempRs = GetDataOutParamsSP("usp_Ica_ExpertOriginalIdSelect", Array( _
...
sProjectKeywordsType=sSearchStringType
	...
	If sProjectKeywords>"" Then
		...
		For i=1 To UBound(arrKeywords)
			If Len(arrKeywords(i)) = 1 Then
				If arrKeywords(i) <> ")" _
				And arrKeywords(i) <> "(" _
				Then
					sProjectKeywords = Replace(sProjectKeywords, " " & arrKeywords(i) & " ", " ")
				End If
			End If
		Next
	...
	' Boolean expression
		Else
			' Replacing all the variants of the word "and not" to "AND NOT" and then placing quotes
			sProjectKeywords = Replace(sProjectKeywords, " AND NOT ", " AND_NOT ",1,1000,1)
...
Function HtmlEncode(AInput)
	Dim sResult, sTag
	sResult = Server.HtmlEncode(AInput)

	Dim aAllowedHtmlTags
	aAllowedHtmlTags = "strong br b"
	Dim arrAllowedHtmlTags
 	arrAllowedHtmlTags = Split(aAllowedHtmlTags, " ")

	For Each sTag in arrAllowedHtmlTags
		sResult = Replace(sResult, "&lt;" & sTag & "&gt;", "<" & sTag & ">")
		sResult = Replace(sResult, "&lt;/" & sTag & "&gt;", "</" & sTag & ">")
	Next
