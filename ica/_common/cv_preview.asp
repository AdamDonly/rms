...
If bCvValidForMemberOrExpert > 0 Then
	' 38 - View CV Full
	iLogResult = LogActivity(38, "CVID=" & Cstr(iCvID) & " Format: General", "", "")
Else
	' 37 - View CV Anonymously
	iLogResult = LogActivity(37, "CVID=" & Cstr(iCvID) & " Format: General", "", "")
End If
%>
...
<body>
	...
	If Not bIsMyCV Then 
		...
		<span class="service_title">Curriculum Vitae.</span> Expert ID: <% =objExpertDB.DatabaseCode %><%=iCvID%>
		<br /><span style="font-size:0.7em;text-transform:none;color:#999">Contact details free version</span>
		</div>
	<%
	End If
...
' Professional experience
	...
	ShowExpertsBlockSubTitle "98%", "52", "ex"  & bCvValidForMemberOrExpert
	ShowUserNoticesViewText GetLabel(sCvLanguage, "Key qualifications"), HighlightKeywords(RemoveContactDetails(sKeyQualification), sSearchKeywordsHighlight)
	ShowUserNoticesViewText GetLabel(sCvLanguage, "Other skills"), HighlightKeywords(RemoveContactDetails(sOtherSkills), sSearchKeywordsHighlight)
...
' Employment records
	...
	WriteDataTitle GetLabel(sCvLanguage, "Employment record and projects completed within the past 10 years")
	While Not objRsExpWke.Eof
		' In preview mode show the last 10 years of experience only. 
		If DateDiff("yyyy", objRsExpWke("wkeEndDate"), Now())<=10 Then
			ShowExpertsBlockSubTitle "98%", "52", "ex"  & bCvValidForMemberOrExpert
			If Not (IsNull(objRsExpWke("wkeStartDate")) Or IsNull(objRsExpWke("wkeEndDate"))) Then
				...
				iDurationYears = DateDiff("yyyy", objRsExpWke("wkeStartDate"), objRsExpWke("wkeEndDate"))
				sDuration = ""
			...
			On Error Resume Next
				ShowUserNoticesViewText GetLabel(sCvLanguage, "Project title"), "<b>" & HighlightKeywords(RemoveContactDetails(objRsExpWke("wkePrjTitleEng")), sSearchKeywordsHighlight) & "</b>"
				...
				ShowUserNoticesViewText GetLabel(sCvLanguage, "Description of tasks"), HighlightKeywords(ConvertText(RemoveContactDetails(objRsExpWke("wkeDescriptionEng"))), sSearchKeywordsHighlight)
			On Error GoTo 0
...
%>
	...
	<div id="rightspace">
		...
		<!-- #include file = "cv_language_select.asp" -->
	</div>
	<!-- footer -->
