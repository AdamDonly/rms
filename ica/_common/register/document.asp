...
<body>
	...
	<input type="hidden" name="document" value="<% =objDocument.UID %>">
		...
		<% If InStr(sScriptFullName, "/mycv/") <= 0 Then %>
		<tr>
		...
		<td class="value blue">
			<select style="width:328px; height:22px;" name="document_typeid">
			<option></option>
			<% 
			Dim objDocumentTypeList 
			Set objDocumentTypeList = New CDocumentTypeList
			objDocumentTypeList.LoadData
			objDocumentTypeList.ShowSelectItems objDocument.TypeID
			%>
			</select>
		</td>
		</tr>
		...
		<a href="javascript:deleteDocument(<% =iExpertID %>, '<% =objDocument.UID %>')" class="red-button next-btn w125">Delete document</a>
		<% Else %>
