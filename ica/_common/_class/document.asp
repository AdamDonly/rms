...
<!--#include file="document_type.asp"-->
<%
Const aDocumentTypeIdCv = 1
Const aDocumentTypeIdMethodologySupport = 112
...
Class CDocument
	...
	Public Function SaveForm(AExpertID, byref AFieldSet)
		If IsObject(AFieldSet) Then
			...
			TypeID = CheckIntegerAndNull(AFieldSet("document_typeid"))
			Text = NULL
...
Class CDocumentList
	...
	Public Function LoadDocumentListByExpertID(AExpertID, ADocumentTypeName, ADocumentTypeID)
		LoadData "usp_ExpertDocumentListSelect", Array( _
			Array(, adInteger, , AExpertID), _
			Array(, adVarchar, 255, ADocumentTypeName), _
			Array(, adInteger, , ADocumentTypeID) _
			)
	End Function

	Public Sub ShowEditListItems()
		Dim i, iScore
		If FCount > 0 Then
			For i = 0 To FCount - 1
			%>
				<p style="margin-bottom: 8px;">
				<% =Items(i).ScoringCriteria.Name %> <span class="ttip ttipright" data-tooltip="<% =Items(i).ScoringCriteria.Description %>"><i class="icon-info-circled"></i></span><br>
				<select class="contractor_score highligh_yellow_input" size="1" name="score_<% =ExpertExperienceID %>_<% =Items(i).ScoringCriteria.ID %>" id="score_<% =ExpertExperienceID %>_<% =Items(i).ScoringCriteria.ID %>" style="width: 50px;">
					<option value="0"></option>
					<% Dim iCompanyScore
					iCompanyScore = CheckIntegerAndZero(Items(i).Score)
					For iScore = 10 To 1 Step -1 %>
					<option value="<% =iScore %>"<% If iCompanyScore = iScore Then %> selected<% End If %>><% =iScore %></option>
					<% Next %>
				</select>
				</p>
			<% 
