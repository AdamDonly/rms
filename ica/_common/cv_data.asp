...
Set objTempRs=GetDataRecordsetSPWithConn(objConnCustom, "usp_ExpertSelect", Array( _
	...
	' Redirect to the homepage if expert marked as deleted / removed or doesn't exist in the database
			'Response.Write(iCvID & "|" & objExpertDB.ID)
			'Response.End
		Response.Redirect "/"
		Response.End
	End If
...
If objExpertTopExpertDBList.Count > 0 Then
	...
	Dim bMyCompanyExpertPrimaryOwner, _
	bMyCompanyExpertOtherOwner, _
	bMyCompanyTopExpertOwner

	bMyCompanyExpertPrimaryOwner = 0
	bMyCompanyExpertOtherOwner = 0
	bMyCompanyTopExpertOwner = 0

	If iUserCompanyID = objExpertDB.Company.ID Then
		bMyCompanyExpertPrimaryOwner = 1
		bMyCompanyExpertOtherOwner = 1
	End If
	If (bCvValidForMemberOrExpert = 1 Or bCvValidForMemberOrExpert = 5) Then
		bMyCompanyExpertOtherOwner = 1
	End If
	If bMyCompanyExpertOtherOwner = 0 Then
	...
	' Cases 1, 3, 5
	If bMyCompanyTopExpertOwner = 0 _
	And bMyCompanyExpertOtherOwner = 0 _
...
Sub ShowExpCVFeatureBox
	...
	bMyCompanyCircleExpert = GetExpertCompanyCircleByUid(sCvUID, iUserCompanyID, iUserID)
	...
	iMyCompanyTopExpertStatus = GetExpertCompanyTopExpertByUid(sCvUID, iUserCompanyID, iUserID)
	...
	If (bCvValidForMemberOrExpert = 1 Or bCvValidForMemberOrExpert = 5) Then
		...
		If bMyCompanyCircleExpert Then 
			...
			<% 
			ElseIf iMyCompanyTopExpertStatus = 2 Then %>
	...
	%>
		...
		<% 	If objExpertDBOtherList.Count > 0 Then
			...
			<b><a href="mailto:<% =objExpertDB.ContactEmail %>"><% =objExpertDB.DatabaseTitle %></a></b>,
			<% 	While iExpertDBOtherLoop < objExpertDBOtherList.Count %>
				<b><a href="mailto:<% =objExpertDBOtherList.Item(iExpertDBOtherLoop).ContactEmail %>"><% =Trim(objExpertDBOtherList.Item(iExpertDBOtherLoop).DatabaseTitle) %></a></b><% 
	...
	If sScriptFileName="cv_preview.asp" Then
		...
		If objExpertTopExpertDBList.Count > 0 Then
				...
				<div class="content">
					<p class="sml"><img src="/image/file_top.gif" width=18 height=17 border=0 hspace=3 align="left"><strong><% =objExpertTopExpertDBList.Item(0).DatabaseTitle %> Top Expert</strong></p>
	
					<p>In order to get contact details for this expert please contact:</p>
					<p><b><a class="list" href="mailto:<% =objExpertDB.ContactEmail %>"><% =objExpertDB.DatabaseTitle %>: <% =objExpertDB.ContactName %></a></b></p>
		...
		<% Else %>
			...
			If objExpertDBOtherList.Count > 0 Then
				...
				While iExpertDBOtherLoop < objExpertDBOtherList.Count
				%>
			<br /><p><b><a class="list" href="mailto:<% =objExpertDBOtherList.Item(iExpertDBOtherLoop).ContactEmail %>"><% =objExpertDBOtherList.Item(iExpertDBOtherLoop).DatabaseTitle %>: <% =objExpertDBOtherList.Item(iExpertDBOtherLoop).ContactName %></a></b><br />
