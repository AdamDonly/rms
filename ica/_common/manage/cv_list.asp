...
<body>
	...
	<div id="content" class="searchform">
		<link rel="stylesheet" type="text/css" href="../../web/css/vendor/bootstrap/bootstrap.css">
...
<div class="frame blue" align="center">
	...
	<td colspan="<% If iShowRemoved=1 Then %>9<% Else %>8<% End If %>" style="padding-left:0;">
		...
		<input type="submit" id="search" value="Search" class="red-button" />&nbsp;&nbsp;
	...
	<tr class="header-row">
		<th <%If sOrderBy="I" Then Response.Write " bgcolor=""#99CCFF""" %> width="40" align="center">&nbsp;<% If sOrderBy<>"I" Then %><a style="color: #FFF !important;" href="<%=sScriptFileName & ReplaceUrlParams(sTempParams, "ord=I") %>"><u><% End If %>Expert&nbsp;ID</u></a>&nbsp;&nbsp;</th>
		<th width="30%" <%If sOrderBy="A" Then Response.Write " bgcolor=""#99CCFF""" %>><% If sOrderBy<>"A" Then %><a style="color: #FFF !important;" href="<%=sScriptFileName & ReplaceUrlParams(sTempParams, "ord=A") %>"><u><% End If %>Name,&nbsp;First&nbsp;name&nbsp;(Title)</a></th>
		<th width="50" <%If sOrderBy="R" Then Response.Write " bgcolor=""#99CCFF""" %>><% If sOrderBy<>"R" Then %><a style="color: #FFF !important;" href="<%=sScriptFileName & ReplaceUrlParams(sTempParams, "ord=R") %>"><u><% End If %>Registered</th>
		<th width="50" <%If sOrderBy="U" Then Response.Write " bgcolor=""#99CCFF""" %>><% If sOrderBy<>"U" Then %><a style="color: #FFF !important;" href="<%=sScriptFileName & ReplaceUrlParams(sTempParams, "ord=U") %>"><u><% End If %>Modified</th>
		<th width="50" <%If sOrderBy="E" Then Response.Write " bgcolor=""#99CCFF""" %>><% If sOrderBy<>"E" Then %><a style="color: #FFF !important;" href="<%=sScriptFileName & ReplaceUrlParams(sTempParams, "ord=E") %>"><u><% End If %>Last&nbsp;experience</th>
	<!--	
	...
	<% While Not objTempRs.Eof And iCurrentRow<objTempRs.PageSize %>
		...
		<td><a class="list" style="color: #b00 !important;" href="../register/register6.asp?uid=<% =objTempRs("uid_Expert") %>" target=_blank><% =sExpertFullName %></a></td>
						...
						If Not IsNull(objTempRs2("CommentorIcaUserId")) Then
							If CStr(iUserID) = CStr(objTempRs2("CommentorIcaUserId")) Then
								...
								If bCanAddComment = 1 Then
									%>
										<a href="javascript:void(0)" class="icon-edit-comment" data-expUid="<% =objTempRs("uid_Expert") %>" 
											...
											data-isPublic="0" title="Edit comment">
												<img src="<% =sHomePath %>image/vn_updt.gif" hspace="6" vspace="0" alt="" />
										</a>
										<div class="comment" data-commentorUserId="<%=objTempRs2("id_User_Commentor") %>" data-expUid="<% =objTempRs("uid_Expert") %>">
											<span class="comment-title red">ME:</span>
											<br/>
											<div class="my-comment"><% =objTempRs2("Comment") %></div>
										</div>
									<% 
								End If
								sIsMyCommentFound = 1
		...
		<td>
			<div class="sml">
				<div style="display: inline-block;padding-bottom: 5px;">
					<a href="../register/register6.asp?uid=<% =objTempRs("uid_Expert") %>" class="list">
						<%= objTempRs("Lng") %>
					</a>
				</div>
				
				<% If Not IsNull(objTempRs("id_Expert_Fra")) And objTempRs("Lng") <> "Fra" Then %> 				
					<div style="display: inline-block;">
						<a href="../register/register6.asp?uid=<% =objTempRs("uid_Expert_Fra") %>" class="list" data-id="<%= objTempRs("id_Expert_Fra") %>">
							Fra
						</a>
					</div>
				<% End If %>
				<% If Not IsNull(objTempRs("id_Expert_Spa")) And objTempRs("Lng") <> "Spa" Then %>
					<div style="display: inline-block;">
						<a href="../register/register6.asp?uid=<% =objTempRs("uid_Expert_Spa") %>" class="list" data-id="<%= objTempRs("id_Expert_Spa") %>">
							Spa
						</a>
					</div>
				<% End If %>
				
				<% If objTempRs("id_Database") & "-" = objUserCompanyDB.DatabaseCode Then %>
					<div class="addButton" onclick="showAddLinkProfile(<%= objTempRs("id_Expert") %>, '<%= objTempRs("Lng") %>');">
						Link Details
					</div>
				<% End If %>
			</div>
...
<style>
    .expertItem td { border: 1px solid grey; }
    .btn { border: 1px solid; }
    .btn--selected { background: lightblue; }
    .addButton { cursor: pointer; text-decoration: underline; color: #EF793F; display: block; }
    .selectedLang { border: 1px solid; background: #D3E9E7; }
	.linkbuttons { width: 100px; height: 23px; line-height: 10px; }
	
</style>
    

	<div id="modalContainer">
		<div class="modal fade modal-as" id="popup" tabindex="-1" aria-labelledby="editDailyTenderAlertsLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header text-center">
						<p>Link Cv's</p>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<i class="material-icons mdi mdi-close-circle" aria-hidden="true"></i>
						</button>
					</div>
					<div class="modal-body">
						<div class="text-center">
							<div class="container">
								<div class="loaderContainer ml-25">
	...
	<script src="../../web/js/vendor/jquery/jquery-2.0.3.min.js" type="text/javascript"></script>
	<script src="../../web/js/vendor/bootstrap/bootstrap.js" type="text/javascript"></script>
	
	<script>
		var database = "<%= objUserCompanyDB.DatabasePath %>";
		var databaseId = "<%= objUserCompanyDB.DatabaseCode %>";
		console.log("[db]" + database)
	
		function showAddLinkProfile(expertId, language) {
			$('#popup').modal();
	
			var id = parseInt(expertId);
			console.log('[lng]: ' + language)
	
			$.ajax({
				url: 'get_all_profiles.asp', 
				method: 'GET',
				data: { expertId: id, language: language, database: database, databaseId: databaseId },
				success: function (data) {
					$('#popup .modal-body .container').html(data);
				}, 
