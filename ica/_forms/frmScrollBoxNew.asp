		...
		if bShowLeftMenu = 1 Then
			%><div class="filter_table_filters">
				...
				<ul><% 
                    ...
                    Set objTempRs =  GetDataRecordsetSPNoParams("usp_DatContinentSelect") 
				j = 0
...
Sub ShowDonScrollBox(sBoxTitle, sLeftMenuTitle, bShowLeftMenu, bShowAll, bShowTotal, bDonorsDelimited)
	...
	<div id="divDonSelector" class="filter_table">
		...
		<%
		If bShowAll = 1 Then 
			%><div class="filter_table_main_obj"><label class="filter_table_checkbox"><input type="checkbox" name="don_0" id="don_0" onclick="RDon(0, 0)" value="on" /></label><label class="option_label_text" for="don_0">&nbsp;<span>Select all funding agencies</span></label></div>
			<% 
		End If
		%>
		<ul>
...
Sub ShowSctScrollBox(sBoxTitle, sLeftMenuTitle, bShowLeftMenu, bShowAll, bShowTotal)
	...
	<div id="divSctSelector" class="filter_table">
		...
		For i = 0 To aExF - 1
			...
			%><div id="header_sct_<%=aExFCode(i)%>" class="filter_table_main_obj">
				...
				<label class="option_label_text" for="msct_<%=aExFCode(i)%>"><span><% =aExFInfo(i) %></span><br/>[select all sub-sectors]</label>
			</div>
			<ul>
			<%
			For j = 0 To aExT - 1 
				If aExTSrch(j) = aExFCode(i) Then
					...
					%><li>
						...
						<label class="option_label_text" for="cou_<%=aExFCode(i)%>_<%=aExTCode(j)%>"><% =aExTInfo(j) %></label>
					</li>
					<%
				End If
			Next
	...
	<% If InStr(sScriptFileName, "exp_search") > 0 Then %>
		...
		<div style="float:left;padding: 2px 5px 5px 15px;">
			<label class="filter_table_checkbox">
