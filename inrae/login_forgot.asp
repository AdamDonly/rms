...
<html>
<head>
<title><% =GetLabel(sCvLanguage, "Forgot your login or password?") %></title>
...
<% ShowTopMenu %>
	...
	<% If bCorrectEmail=1 Then
		ShowMessage GetLabel(sCvLanguage, "Your login and password has now been sent"), "info", 450
		%>
		<br>
	...
	<% Else
		If sUserEmail>"" Then
			ShowMessage GetLabel(sCvLanguage, "The email address is not available"), "error", 450
		End If

		ShowInputFormHeader 450, GetLabel(sCvLanguage, "FORGOTTEN PASSWORD OR LOGIN") %>
		<form method="post" action="<% =sScriptFileName %><%=sParams%>" name="LoginForm">
		<% ShowInputFormElement 450, "Email", "text", "user_email", "", 50, 255, 0, ""
