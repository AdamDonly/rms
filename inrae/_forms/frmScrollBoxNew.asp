...
Sub ShowCouScrollBox(sBoxTitle, sLeftMenuTitle, bShowLeftMenu, bShowAll, bShowRegions, bShowCountries, bShowTotal)
	...
	<div id="divCouSelector" class="filter_table">
		<% ' title:
		If sBoxTitle > "" Then Response.Write("<div class=""filter_table_header"" style=""background: " & colFormHeaderMain & """><h3>" & sBoxTitle & "</h3></div>")
		...
		if bShowLeftMenu = 1 Then
			...
			<div class="filter_table_filters"><%
				...
				<ul>
					...
					<a href="javascript:void(0);" id="don_2" onclick="ScrollToBlock('don_2', 'divDonList')"><li><p>Other funding agencies <span id="DonTotal_2" class="filter_table_nbr_selected red"></span></p></li></a>
				<%
				' main total:
				If bShowTotal = 1 Then 
			...
			If aOrgMainDonor(j) = 0 And bDonorsDelimited = 0 Then 
				...
				If sScriptFileName = "register_bsc.asp" Or sScriptFileName = "update_bsc.asp" Then 
					%><div id="header_don_2" style="height:2px; background-color: #ECECEC; color: #1A466A; margin: 5px 0;"></div>
						...
						<label class="option_label_text"><span>Other funding agencies</span></label>
					</div>
					<%
				else
					%><div id="header_don_2" style="height:2px; background-color: #ECECEC; color: #1A466A; margin: 5px 0;"></div>
			...
			For j = 0 To aExT - 1 
				If aExTSrch(j) = aExFCode(i) Then
					...
					%><li>
						...
						<label class="option_label_text" for="sct_<%=aExFCode(i)%>_<%=aExTCode(j)%>"><% =CutStringNSplit(aExTInfo(j), 55, "<br>") %></label>
					</li>
					<%
				End If
			Next
