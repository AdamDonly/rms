...
%>
	...
	<td bgcolor="<%=colFormHeaderTop%>" width=169 valign="top">
		...
		If i>0 Then
		  'ts=ts+aGZnScroll(i-1)*21+34+tt-is_nav*(aGZnScroll(i-1)*2+3+tt/6)
		  ts=ts+aGZnScroll(i-1)*21+34+tt-is_nav*(aGZnScroll(i-1)*2+3+tt/6)-is_chrome*(aGZnScroll(i-1)*1.07)+is_mozilla*(aGZnScroll(i-1)*2.3)
		  tt=0
		End If %>
...
%>
	...
	<td bgcolor="<%=colFormHeaderTop%>" width=169 valign="top">
		...
		<p class="fsml"><img src="x.gif" width=3 height=7 vspace=2 hspace=4 align="left"><a class="dk" name="MDon1" id="MDon1" href="javascript:;" onClick="scroll(9000,2);scroll(-9000,2);scroll(228,2);noScroll();"><span name="MDonText1" id="MDonText1">Other funding agencies</span></a></p>
		<% Else %>
		...
		<% FOR j=0 to aOrg-1 %>
		  <% If aOrgMainDonor(j)=0 And bDonorsDelimited=0 Then 
			...
			If sScriptFileName="register_bsc.asp" Or sScriptFileName="update_bsc.asp" Then %>
				<p class="<%=cssScrllSubTitle%>">&nbsp;&nbsp;&nbsp;Other funding agencies</p>
			<% End If %>
...
%>
	...
	<td bgcolor="<%=colFormHeaderTop%>" width=169 valign="top">
		...
		FOR i=0 to aExF-1
		If i>0 Then
		  'ts=ts+aExFScroll(i-1)*21+34+tt+aExFShift(i-1)*13-is_nav*(aExFScroll(i-1)*2+aExFShift(i-1)*1+3+tt/6)
		  ts=ts+aExFScroll(i-1)*21+34+tt+aExFShift(i-1)*13-is_chrome*(aExFScroll(i-1)*1+aExFShift(i-1)*0.35)+is_mozilla*(aExFScroll(i-1)*2.2+aExFShift(i-1)*2.15)
		  tt=0
		End If %>
